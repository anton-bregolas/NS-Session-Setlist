...
// Import deflate compression algorithm
...
export async function loadTuneBookItem(currentTuneBook, itemNumber, passedUrl) {
    ...
    // Fallback case for Session DB with full https links
    if (theURL.startsWith(`https://`)) {

        theURL =
            localStorageOk() && +localStorage.abcToolsUseLiteEditor === 1?
                theURL.replace(`https://michaeleskin.com/abctools/abctools.html`, `https://abc.tunebook.app/abctools.html`) :
                theURL;

    // Session DB for NS Session Setlist v.1.2+ and later
    } else {

        theURL = checkLinkForPreferredEditor(theURL);
    }
...
function openInAbcTools() {
    ...
    if (lastURL != "") {
        ...
        lastURL = checkLinkForPreferredEditor(lastURL);
...
// Ensure last ABC Tools tune link is opened in the currently selected preferred editor

function checkLinkForPreferredEditor(url) {

    if (localStorageOk() &&
    +localStorage.abcToolsUseLiteEditor === 1 &&
    !url.startsWith(`https://abc.tunebook.app/`)) {

        return `https://abc.tunebook.app/abctools.html?${url.split('.html?')[1]}`
    }
    ...
    if (localStorageOk() &&
        ...
        return `https://michaeleskin.com/abctools/abctools.html?${url.split('.html?')[1]}`
    }
